 ...
 */
class PKCS12FileLoader extends FileKeyStoreLoader {
    private static final Logger LOG = LoggerFactory.getLogger(PKCS12FileLoader.class);
    ...
    public KeyStore loadKeyStore() throws IOException, GeneralSecurityException {
        KeyStore ks = KeyStore.getInstance(PKCS12_KEY_STORE_TYPE);
    ...
    public KeyStore loadTrustStore() throws IOException, GeneralSecurityException {
        KeyStore ts = KeyStore.getInstance(PKCS12_KEY_STORE_TYPE);
        InputStream inputStream = null;
        try {
